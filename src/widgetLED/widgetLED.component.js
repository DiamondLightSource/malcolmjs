...
const stylesNoTheme = (colorBorder, colorCenter) => ({
  border: {
    fill: colorBorder,
  },
  center: {
...
const WidgetLED = props => {
  const configuredStyle = stylesNoTheme(props.colorBorder, props.colorCenter);
  return (
    <div className={props.classes.div}>
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width={configuredStyle.svg.width}
        viewBox="-24 -24 96 96"
      >
        <path d={outlinePath} fill={configuredStyle.border.fill} />;
        {props.LEDState ? (
          <circle cx="24" cy="24" r="12" fill={configuredStyle.center.fill} />
        ) : null}
      </svg>
    </div>
  );
